...
#Setup of argparse
parser = argparse.ArgumentParser(description='This tool can be used to analyze a virus mutation table.', add_help=False, epilog='Thank you for using this analysis tool!\n\n(C) 2021\nSebastian Hupfauf')
...
optionalNamed.add_argument('-c1', '--clusters1', action='store', default=3, type=int, help="Number of Clusters in the first cluster analysis. This is important to correctly destinguish between mutations of interest (MOIs) and background noise [Default: 3]")
...
optionalNamed.add_argument('-rf', '--reference_file', action='store', default='auto', type=str, help="Reference file for the construction of the FASTA file when using the 'pangolin' mode from --assignment. Provide 'auto' to automatically download the suggested SARS-COV2 file from NCBI. [Default: auto")
...
optionalNamed.add_argument('-u', '--usher', action='store_true', help="Use UShER model instead of default pangoLEARN when assigning clusters with pangolin. ATTENTION: This option is only needed when --assignment is set to 'pangolin'!")
optionalNamed.add_argument('-ft', '--frequency_threshold', action='store', default=0.05, type=float, help="Minimum mean frequency (= relative abundance) of all mutations of a cluster after the first cluster analysis. Mutations of clusters fullfilling this criterion are added to the list of MOIs, while the others are categorized as bias. ATTENTION: Input must be a number between 0 and 1! [Default: 0.05]")
...
optionalNamed.add_argument('-omtr', '--ometric', action='store', type=str, help="Metric used for ordination analysis, available options are: euclidean, minkowski, cityblock, seuclidean, sqeuclidean, cosine, correlation, hamming, jaccard, chebyshev, canberra, braycurtis, mahalanobis, yule, matching, dice, kulsinski, rogerstanimoto, russellrao, sokalmichener, sokalsneath, and wminkowski. If you want to use multiple metrics, separate them by a ',', e.g. 'euclidean,braycurtis'. Leave this parameter out if you do not want to compute an ordination analysis!")
optionalNamed.add_argument('-mv', '--meta_variable', action='store', type=str, default='None', help="Metadata variable that is used to color the datapoints in the PCoA plot(s) and to create bar charts (VOC composition) specifically for this parameter. If you want to use multiple variables, separate them by a ',', e.g. 'sample_source_location_state,ep_week'. Provide 'None' if you do not want to color the data points and create specific charts. [default: None]")
...
optionalNamed.add_argument('-c', '--colors', action='store', default='standard', type=str, help="Define colors for all plots (dendrogram, bar charts). You can choose between all common Python colormaps (e.g. https://matplotlib.org/stable/tutorials/colors/colormaps.html), e.g. 'viridis'. Use 'standard' for the default color style of the clustering tool! [Default: standard]")
